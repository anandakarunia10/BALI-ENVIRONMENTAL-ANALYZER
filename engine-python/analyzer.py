...
def get_uhi_data(lat, lng, radius, date_start, date_end):
    ...
    try:
        # 1. Buat Poligon berdasarkan Radius (Point -> Buffer -> Bounds)
        point = ee.Geometry.Point([lng, lat])
        region = point.buffer(radius * 1000).bounds()

        # 2. Ambil Koleksi Landsat 8 Level 2 Collection 2
        collection = ee.ImageCollection("LANDSAT/LC08/C02/T1_L2") \
            .filterBounds(region) \
            .filterDate(date_start, date_end) \
            .sort('CLOUD_COVER')

        # Ambil citra pertama (paling jernih)
        image = collection.first()

        # Pengecekan jika citra tidak ditemukan
        ...
        # --- 5. KONFIGURASI VISUALISASI (Layer Suhu) ---
        vis_params = {
            'min': 25, 
            'max': 45, 
